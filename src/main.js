...
async function main() {
  ...
  const response = await fetch("./objs/memoryCard/Memorycard.obj")
  const text = await response.text()
  const objBackground = parseOBJ(text);

  const parts = objBackground.geometries.map(({data}) => {
  ...
  function updateCameraAngle(event, ui) {
    cameraAngleRadians = degToRad(ui);
